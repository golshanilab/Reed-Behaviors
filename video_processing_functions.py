...
def crop_frame_with_data(frame, crop_data):
    try:
        Xs = [crop_data['x0'][0], crop_data['x1'][0]]
        Ys = [crop_data['y0'][0], crop_data['y1'][0]]
...
def crop_and_save_video(input_video, output_folder, crop):
    # Open the video file
    ...
    fxmin, fxmax = int(min(Xs)), int(max(Xs))
    fymin, fymax = int(min(Ys)), int(max(Ys))
    crop_width = fxmax - fxmin
    crop_height = fymax - fymin

    # Create a VideoWriter object to save the output video
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    output_video_path = os.path.join(output_folder, os.path.basename(input_video))
    ...
    while True:
        ...
        # Crop the frame based on the BoxEdit data
        cropped_frame = crop_frame_with_data(frame, crop.data)
        ...
        if cv2.waitKey(1) & 0xFF == 27:  # Press 'Esc' to exit the video display
            break
