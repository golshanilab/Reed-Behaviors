...
def load_videos_Samara(parent_folder, subFolder, dtype: Union[str, type] = np.float64, downsample: Optional[dict] = None, downsample_strategy="subset", post_process: Optional[Callable] = None) -> xr.DataArray:
    
    parent_path = parent_folder
    vlist = []  # List to store AVI file paths

    vpath = os.path.join(parent_folder, subFolder)
    for v in os.listdir(vpath):
        if v.lower().endswith(".avi"):
            vlist.append(os.path.join(vpath, v))

    vlist = natsorted(vlist)
    print(vlist)
    
    # Replace these with your actual video loading functions
...
def find_avi_files(parent_path, animalIds, subFolder, temp_save_path=None):
    avi_files = []  # List to store AVI file paths
    dpath = None  # Variable to store the target folder path

    for animalId in animalIds:
        animal_file = animalId
        animalId_path = os.path.join(parent_path, animalId)

        if os.path.exists(animalId_path) and os.path.isdir(animalId_path):
            print("Found target folder:", animalId_path)
            dpath = animalId_path

            # Iterate through subfolders
            subFolder_path = os.path.join(dpath, subFolder)
                ...
                for root, dirs, files in os.walk(subFolder_path):
                    for file in files:
                        ...
                        if file.lower().endswith(".avi"):
                            ...
                            avi_files.append(avi_file_path)
                break
            else:
                continue
            break
    ...
    try:
        if temp_save_path:
            minian_ds_path = os.path.join(temp_save_path, "minian")
            intpath = os.path.join(minian_ds_path, "minian_intermediate")
            
        else:
            raise NameError("temp_save_path is defined but has no value")
    except NameError:
    ...
    if avi_files:
        return avi_files, avi_file_path, dpath, minian_ds_path, intpath
    else:
        print("No AVI files found.")
        return None

def convert_to_filename_save_to_different_directory(dpath, save_path, optional_filename=None):
    # dpath is path where file came from
    # save_path is path to save file to
    # optional_filename is an optional parameter

    # Extract the last part of the parent_path
    last_part = os.path.basename(os.path.normpath(dpath))

    # Include optional_filename if provided, otherwise use an empty string
    optional_filename = optional_filename or ""

    # Combine with ".nc" and optional_filename to create the new filename
    new_filename = last_part + optional_filename + ".nc"

    # Combine the new filename with the original parent_path
    new_path = os.path.join(save_path, new_filename)  # this would save the .nc file to the parent_path
